...
window.loadSiteData = async () => {
  ...
  try {
    const [settings, programs, products, services, media] = await Promise.all([safeFetch(`data/settings.json?v=${v}`), safeFetch(`data/programs.json?v=${v}`), safeFetch(`data/products.json?v=${v}`), safeFetch(`data/services.json?v=${v}`), safeFetch(`data/media.json?v=${v}`)]);
  ...
  function setup() {
    ...
    function tick() {
      ...
      for (const d of dots) {
        ...
        const frontFade = stretchVal > 1.06 ? Math.max(0, 1 - Math.max(0, along - fwdClip) / fwdClip) : 1;
...
(function initAuroraBackground() {
  ...
  function setup() {
    ...
    wrap.style.cssText = 'position:fixed;inset:0;z-index:45;pointer-events:none';

    // Left — upper blob
    const b1 = document.createElement('div');
    b1.style.cssText = 'width:50vw;height:50vw;top:-15%;left:-25%;' + 'background:var(--brand-500);opacity:0.13;' + 'animation:orb-drift-a 20s ease-in-out infinite';

    // Left — lower blob (offset timing)
    const b2 = document.createElement('div');
    b2.style.cssText = 'width:40vw;height:40vw;bottom:-10%;left:-20%;' + 'background:var(--brand-400);opacity:0.09;' + 'animation:orb-drift-b 26s ease-in-out infinite';

    // Right — upper blob
    const b3 = document.createElement('div');
    b3.style.cssText = 'width:50vw;height:50vw;top:-15%;right:-25%;' + 'background:var(--brand-500);opacity:0.11;' + 'animation:orb-drift-b 22s ease-in-out infinite';

    // Right — lower blob
    const b4 = document.createElement('div');
    b4.style.cssText = 'width:40vw;height:40vw;bottom:-10%;right:-20%;' + 'background:var(--brand-400);opacity:0.08;' + 'animation:orb-drift-a 28s ease-in-out infinite';
    wrap.appendChild(b1);
    wrap.appendChild(b2);
    wrap.appendChild(b3);
    wrap.appendChild(b4);
